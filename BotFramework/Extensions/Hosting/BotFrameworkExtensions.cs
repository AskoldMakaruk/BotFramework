...
public static class BotFrameworkExtensions
{
                                               ...
                                               bool                                    isTesting = false)
    {
        ...
        .ConfigureContainer<AppBuilder>((context, appBuilder) =>
        {
            ...
            new AppRunnerService((BotDelegate)context.Properties[AppKostyl],
                provider.GetService<ITelegramBotClient>()!, provider.GetService<ILogger<AppRunnerService>>()!));

            if (isTesting)
            {
                appBuilder.Services.AddSingleton(_ =>
                new DebugDelegateWrapper((BotDelegate)context.Properties[AppKostyl]));
            }
        })
        .UseConsoleLifetime();
    ...
    public record DebugDelegateWrapper(BotDelegate App);
}